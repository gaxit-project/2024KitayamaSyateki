...
public static class Poly2Mesh {
	...
	public static Mesh CreateMesh(Polygon polygon) {
        ...
        Virtence.VText.Tesselation.Poly2Tri.Polygon poly = new Virtence.VText.Tesselation.Poly2Tri.Polygon(ConvertPoints(polygon.outside, polygon.rotation));

		// Convert each of the holes
		foreach (List<Vector3> hole in polygon.holes) {
			poly.AddHole(new Virtence.VText.Tesselation.Poly2Tri.Polygon(ConvertPoints(hole, polygon.rotation)));
		}
