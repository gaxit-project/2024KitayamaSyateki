...
namespace Virtence.VText
{
	...
	internal class BevelBuilderFlat : BevelBuilderStrategy
	{
        ...
        internal BevelBuilderFlat(VTextMeshParameter meshParameter) : base(meshParameter)
        {
        }
        ...
        internal override void AddBevelBackfacesToMesh(ref MeshAttributes meshAttribs)
        {
            ...
            for (int j = 0; j < meshAttribs.VertexAttributes.Count; ++j)
            {
                ...
                foreach (var attr in meshAttribs.VertexAttributes[j])
                {
                    ...
                    // add norms
                    Vector3 bevelFaceNormal = (attr.Normal + Vector3.forward * _meshParameter.Bevel).normalized;
                    meshAttribs.Normals.Add(bevelFaceNormal);
                    meshAttribs.Normals.Add(bevelFaceNormal);
        ...
        internal override void AddBevelFrontFacesToMesh(ref MeshAttributes meshAttribs)
        {
            ...
            int offset = meshAttribs.Indicies[0].Max() + 1;
            ...
            for (int j = 0; j < meshAttribs.VertexAttributes.Count; ++j)
            {
                ...
                foreach (var attr in meshAttribs.VertexAttributes[j])
                {
                    ...
                    // add norms
                    Vector3 bevelFaceNormal = (attr.Normal + Vector3.back * _meshParameter.Bevel).normalized;
                    meshAttribs.Normals.Add(bevelFaceNormal);
                    meshAttribs.Normals.Add(bevelFaceNormal);
