...
/// </summary>
public class VTextSetup : UnityEditor.EditorWindow
{
    #region CONSTANTS
    ...
    private Vector2 _welcomeScrollViewPosition = new Vector2();             // the scroll view position in the welcome screen
    private Vector2 _licenseAgreementScrollViewPosition = new Vector2();    // the scroll view position in the license agreement screen
    private Vector2 _fontInstallationScrollViewPosition = new Vector2();    // the scroll view position in the fonts installation screen
    private Vector2 _finishScrollViewPosition = new Vector2();              // the scroll view position in the finish screen
    ...
    private bool _installationStarted = false;                              // determines if the installation of plugins and folders was already started
    private bool _installationSuccessful = false;                           // determines if the installation of plugins and folders was already started
    ...
    private string GetParentFolder(string path, string parentName)
    {
        ...
        return GetParentFolder(dir.Parent.FullName, parentName);
    }
    ...
    {
        DirectoryInfo di = Directory.CreateDirectory(Path.Combine(Application.streamingAssetsPath, "Fonts"));
    ...
    private void InstallFont(string fontname)
    {
        try
        {
            FileInfo fi = new FileInfo(fontname);
            FileUtil.CopyFileOrDirectory(fontname, Path.Combine(Path.Combine(Application.streamingAssetsPath, "Fonts"), fi.Name));
            FileUtil.CopyFileOrDirectory(fontname, Path.Combine(Path.Combine(Application.persistentDataPath, "Fonts"), fi.Name));
        }
        catch (IOException)
        {
        }
